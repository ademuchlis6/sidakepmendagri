...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$tbl = $this->tables['users'];
/*
		$b = 'BINARY ussername =';
		//$c = " ";
		$bx = str_replace("'", "", $b);
		//$d = $bx.$c.'=';
		$query = $this->db->select($this->identity_column . ', usernamxxe, email, id, password, active, last_login')
//		                  ->where($this->identity_column, $identity)
//		                  ->where('username', $identity)//ni kalau mau login pake user name
						  ->where($bx,$identity)//ni kalau mau login pake user name
						  ->limit(1)
						  ->get($this->tables['users']);
*/						  
		$query = $this->db->query("SELECT `$this->identity_column`, `username`, `email`, `id`, `password`, `active`, `last_login` from `$tbl` where BINARY username ='$identity' limit 1" );
	...
	public function user_by_email($email = NULL)
	{
		$this->trigger_events('user');
		...
		$this->limit(1);
		$this->where($this->tables['users'].'.email', $email);
	...
	**/
	...
	public function delete_user_by_email($email)
	{
		...
		$res = $this->db->where('email',$email)->get($this->tables['users'])->row_array();
		$id = $res['id'];
	...
	**/
	...
	function setusername($identity){
		$sql  = "UPDATE users set statlog = '1' where  email  = '$identity'";
		$this->db->query($sql);
	}

	function getgroupid($identity){
		$sql  = "SELECT ug.group_id FROM users_groups ug JOIN users u ON ug.user_id = u.id and u.username = '$identity' order by group_id asc";
